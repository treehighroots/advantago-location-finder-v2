<div class="wrap">
    ...
    <form method="post" action="options.php">
        ...
        <table class="form-table">
            ...
            <tr>
                <th scope="row"><label for="hitlist_page"><?php _e('Page for hitlist view', 'advantago-location-finder'); ?></label></th>
                <td>
                    <select id="hitlist_page" name="alf_settings[hitlist_page]">
                        <option value=""><?php _e('— Select —', 'advantago-location-finder'); ?></option>
                        <?php foreach ($pages as $page) : ?>
                            <option value="<?php echo esc_attr($page->ID); ?>" <?php selected(isset($options['hitlist_page']) ? $options['hitlist_page'] : '', $page->ID); ?>><?php echo esc_html($page->post_title); ?></option>
                        <?php endforeach; ?>
                    </select>
                </td>
            </tr>
            ...
            <tr>
                <th scope="row"><label for="detail_page"><?php _e('Page for detail view', 'advantago-location-finder'); ?></label></th>
                <td>
                    <select id="detail_page" name="alf_settings[detail_page]">
                        <option value=""><?php _e('— Select —', 'advantago-location-finder'); ?></option>
                        <?php foreach ($pages as $page) : ?>
                            <option value="<?php echo esc_attr($page->ID); ?>" <?php selected(isset($options['detail_page']) ? $options['detail_page'] : '', $page->ID); ?>><?php echo esc_html($page->post_title); ?></option>
                        <?php endforeach; ?>
                    </select>
                </td>
            </tr>
            ...
            <tr>
                <th scope="row"><?php _e('Use Mockdata?', 'advantago-location-finder'); ?></th>
                <td>
                    <input type="checkbox" id="use_mockdata" name="alf_settings[use_mockdata]" value="1" <?php checked(isset($options['use_mockdata']) ? $options['use_mockdata'] : 0, 1); ?>>
                    <label for="use_mockdata"><?php _e('Use Mockdata?', 'advantago-location-finder'); ?></label>
                </td>
            </tr>
